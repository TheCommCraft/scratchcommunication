...
char_to_idx = {
    char: str(idx) if len(str(idx)) > 1 else "0" + str(idx) for idx, char in enumerate(chars, 1)
}
...
class BaseCloudSocketClient(ABC):
    ...
    def __init__(
        ...
        *,
        ...
        username: str = "user1000",
        packet_size: int = 220,
        security: Optional[Union[tuple, sec.Security]] = None,
    ):
        pass
...
class AnyCloudSocket(ABC):
    ...
    def __init__(
        ...
        *,
        cloud: CloudConnection,
        packet_size: Union[int, Literal["AUTO"]] = "AUTO",
        security: Union[None, tuple, sec.Security] = None,
    ):
        pass
...
class CloudSocket(VariableCloudSocket):
    ...
    def __init__(
        ...
        *,
        cloud: CloudConnection,
        packet_size: Union[int, Literal["AUTO"]] = "AUTO",
        security: Union[None, tuple[int, int, int], sec.Security] = None,
    ):
        self.security = None
        security_type = None
        security_data: Optional[Union[tuple[int, int, int], tuple[str, str, str]]] = None
        if isinstance(security, tuple) and not isinstance(security, sec.Security):
            security_type = "RSA"
        ...
        if security_type == "EC":
            ...
            self.security = sec.ECSecurity(
                (str(security_data[0]), str(security_data[1]), str(security_data[2]))
            )
        self.cloud = cloud
        self.clients = {}
        self.new_clients = []
    ...
    def listen(self) -> Self:
        ...
        def on_packet(event: Event):
            try:
                ...
                if (
                    not (
                        self._decode(msg_data[:28]).startswith("_connect")
                        or self._decode(msg_data[:28]).startswith("_safe_connect:")
                    )
                    and client in self.clients
                    and not self.clients[client].secure
                ):
                    event.emit(
                        "non_secure_message_part",
                        client=self.clients[client],
                        decoded=self._decode(msg_data),
                        raw=msg_data,
                    )
                    ...
                    event.emit(
                        "non_secure_message",
                        client=self.clients[client],
                        content=self.clients[client].current_msg.message,
                    )
                ...
                if (
                    ...
                    and client in self.clients
                    and self.clients[client].secure
                ):
                    ...
                    affected_client.current_msg.add(
                        decoded := affected_client.encrypter.decrypt(
                            self._decode(msg_data[:-15]), int(msg_data[-15:])
                        )
                    )
                ...
                if self._decode(msg_data[:28]).startswith("_safe_connect:"):
                    ...
                    try:
                        key_parts = [
                            self.key_parts["".join(key_part)]
                            for key_part in batched(msg_data[28:-15], 5)
                        ]
                    except KeyError:
                        raise AssertionError from None
                    c_key = "".join(key_parts)
                    key = self._decrypt_key(c_key)
                    assert str(key).endswith(str(int(msg_data[-15:]))) or str(key).startswith(
                        str(int(msg_data[-15:]))
                    )
    ...
    def _decode(data: Union[str, int]) -> str:
        ...
        for char_pair in zip(data[::2], data[1::2]):
            ...
            try:
                ...
                warnings.warn(
                    f'There was an error in decoding a message: "{data}" has "{char_idx}" which doesn\'t exist.'
                )
        return decoded
    ...
    def accept(
        self, timeout: Union[float, int, None] = 10
    ) -> tuple[BaseCloudSocketConnection, str]:
        """
        Returns a new client
        """
    ...
    def secure_send_to_client(self, data: str, client: BaseCloudSocketConnection):
        ...
        for packet in packets[:-1]:
            ...
            client.set_var(
                name=f"TO_CLIENT_{var}",
                value=int(
                    f"-{encoded_packet}{str(salt).zfill(15)}.{client.client_id}{random.randrange(1000):03}{packet_idx}"
                ),
            )
            var = var % 4 + 1
            packet_idx += 1
...
class CloudSocketConnection(BaseCloudSocketConnection):
    ...
    def __init__(
        ...
        *,
        ...
        self.encrypter = (self.security is not None or self.security) and sec.SymmetricEncryption(
            int(self.security)
        )
